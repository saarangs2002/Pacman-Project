...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for i in range(X_RANGE+Y_RANGE+6):
        allPossibleManhattanDistances.append(i)
    
    variableDomainsDict['Pacman'] = allPossiblePositions
    variableDomainsDict['Ghost0'] = allPossiblePositions
    variableDomainsDict['Ghost1'] = allPossiblePositions
    variableDomainsDict['Observation0'] = allPossibleManhattanDistances
    variableDomainsDict['Observation1'] = allPossibleManhattanDistances
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        #extracting the variables to be eliminated in the order of the list
        for eliminationVariable in eliminationOrder:
            #finding the joint of the factors containing the current variable to be eliminated
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, eliminationVariable)
            #checking if elimination is possible in this factor , i.e. variable to be eliminated is containing more than one unconditioned variables
