...
class QLearningAgent(ReinforcementAgent):
    ...
    def getQValue(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        #finding QValue of a state action pair
    ...
    def computeValueFromQValues(self, state):
        ...
        bestQ = -99999999999
        #finding the utility of state from QValues
    ...
    def computeActionFromQValues(self, state):
        ...
        #finding action with best qValue
        for action in legalActions:
          if(self.getQValue(state,action)>bestQ):
            bestQ = self.getQValue(state,action)
            bestAction = action
        ...
        allBestActions = []
        #chosing an in random from all actions with best qValue
        for action in legalActions:
          if(self.getQValue(state,action)==bestQ):
            allBestActions.append(action)
        
        if(len(allBestActions)!=1):
          bestAction = random.choice(allBestActions)
    ...
    def getAction(self, state):
        ...
        #choosing a random action with a probabilty epsilon
